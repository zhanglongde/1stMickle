...
3.DOM BOM CSSOM 事件
    ...
    事件类型 mouseover与mouseenter区别   模拟双击300ms
            input事件没有keyCode change事件，失去焦点触发
...
1.6个扩展 字正数组像值函
  ...
  函数:箭头函数/默认参数/参数解构与rest参数（是一个数组，避免使用arguments）/函数简洁形式
       默箭简解
2.sspprig lac变mod
  symbol(字符串)
  set与map
...
工作流程
    ...
    联调 测试 上线
前后端配合
    套模板 form表单 跨域 前后端分离:ajax(JSON formdata) WebSocket sse
    cookie oauth
持续集成 交付
    travis jenkins
部署
