...
3.DOM CSSOM 事件
    DOM appendChild insertBefore(node,reference) removeChild
        ...
        querySelector window.getComputedStyle(document.body,null)/style.cssText  classList matches contains(this.contains(e.target))
...
4.函数 类 对象 继承
    函数
        ...
        构造函数 new Function()
    对象 字new工
    继承 3+1:原型链 基类构造函数.apply(arguments) Object.create extends+super
5.正则 2+2+4 5个应用
  正则字面量 new Reg()
