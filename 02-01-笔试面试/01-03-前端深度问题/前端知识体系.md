...
内核
浏览器缓存 4
下载与渲染 域名解析dns/cname/DNSPod 缓存 http://fex.baidu.com/blog/2014/05/what-happen/
...
1.变量与值 类型判断
  ...
  比较 特殊类型的比较:NaN 空数组空对象比较 浮点数比较 _.isEqual深比较&特殊类型的比较
...
3.DOM CSSOM 事件
    DOM appendChild insertBefore(node,reference) removeChild
        ...
        querySelector window.getComputedStyle(document.body,null)  classList matches contains(this.contains(e.target)) document.body.all document.activeElement
...
4.函数 类 对象 继承
    函数
        ...
        2种声明
        3个绑定
        4个应用:高阶函数-偏函数-柯里化(bind/闭包/lodash/箭头函数)-防抖与节流(lodash/setTimeout绑定event.target)
...
自动化 无头浏览器 headless chrome/phantomjs
